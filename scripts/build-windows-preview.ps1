...
& cargo build --locked --manifest-path (Join-Path $repo 'crates\lobby\Cargo.toml')
...
& python (Join-Path $PSScriptRoot 'package-windows.py') --target $TargetDir --love-zip $loveZip --pinpals-zip $gameZip --output $packageDir
