...
async function run(native){
  const nodes=new Map(),draws=[];let scans=0;
  const node=id=>{if(!nodes.has(id))nodes.set(id,{open:true,hidden:true,addEventListener(){},getBoundingClientRect(){return {left:0,top:0,right:400,bottom:600};}});return nodes.get(id);};
  ...
  const context={document:{getElementById:node,createElement(){return canvas;},addEventListener(){}},window:{addEventListener(){},jsQR(){scans++;return scans===2?{data:'https://gamenight.ontola.io/studio#pair=test'}:null;}},URL:TestURL,Image:class{naturalWidth=3840;naturalHeight=2160;async decode(){}},navigator:{},setTimeout(){},clearTimeout(){}};
  ...
  assert.equal(node('qr-result').hidden,false);assert.equal(event.target.value,'');
