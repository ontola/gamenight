...
fn scoring_menu_system(
    ...
    // Tell GameNight the match/set is over the moment we know a game was
    // won, independent of when players finish confirming on this screen —
    // the daemon opens its replay/next-game vote immediately, while we keep
    // rendering the scoreboard.
    #[cfg(not(target_arch = "wasm32"))]
    if game_won {
        if let Some(mut bridge) = world.get_resource_mut::<crate::gamenight::GameNightBridge>() {
            bridge.notify_finished();
        }
    }
