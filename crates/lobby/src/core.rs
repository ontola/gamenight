...
pub struct MatchPlugin {
    ...
    /// GameNight's always-on lobby arena instead of a scored match — see
    /// [`LobbyMode`].
    pub lobby_mode: bool,
}
...
impl SessionPlugin for MatchPlugin {
    fn install(self, session: &mut SessionBuilder) {
        ...
        weapon_drop::session_plugin(session);
        ...
        session.insert_resource(LobbyMode(self.lobby_mode));
        session.set_session_runner(self.session_runner);
    }
}

#[derive(Default)]
pub struct LobbyDefaultMatchRunner {
...
impl SessionRunner for LobbyDefaultMatchRunner {
    ...
    fn restart_session(&mut self) {
        *self = LobbyDefaultMatchRunner::default();
    }
