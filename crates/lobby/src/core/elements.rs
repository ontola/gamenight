...
pub mod mine;
pub mod music_pad;
pub mod music_screen;
pub mod musket;
pub mod next_game_trigger;
pub mod periscope;
pub mod qr_sign;
pub mod sign_in;
pub mod exit_door;
...
pub mod prelude {
    pub use super::{
        ...
        kick_bomb::*, machine_gun::*, mine::*, music_pad::*, music_screen::*, musket::*, next_game_trigger::*, periscope::*, qr_sign::*, sign_in::*, exit_door::*,
        player_spawner::*, slippery::*, slippery_seaweed::*, snail::*, spike::*, sproinger::*,
        stomp_boots::*, sword::*, urchin::*, *,
    };
}
...
/// How much slab there is around a lobby screen on every side.
///
/// The lobby's screens are level geometry — solid platforms with the screen
/// set into the face — so this is the part you actually land on, and it has
/// to read as a thing with edges rather than a picture hanging in the air.
pub const SCREEN_FRAME: f32 = 12.0;

/// How long a lobby button stays visibly pushed in after somebody lands on it.
///
/// The press *is* the feedback — these buttons deliberately don't light up
/// when you merely walk into them, because something that reacts to being
/// walked past looks like it did something when it didn't.
pub const PAD_PRESS_SECONDS: f32 = 0.22;

/// How close a player's feet must be to the top of a button to count as
/// standing on it. The physics parks them exactly on the surface, so this is
/// slack for float error, not a real tolerance.
const LANDING_SLOP: f32 = 4.0;

/// Whoever just *landed* on the button at `centre` this frame, by player
/// index — `None` if nobody did.
///
/// The lobby's buttons (sign-in, next game, the music jukebox) are things you
/// jump onto, not things you loiter on. A floor zone you merely stand in is
/// indistinguishable from one you walked across on your way somewhere else,
/// which is why these used to need a hold timer to tell the two apart — and a
/// hold timer is a chore you perform, not an action you take. Landing is the
/// action: it can't happen by walking, it takes a deliberate jump, and it
/// fires the instant you touch down.
///
/// Measured against the button's top surface rather than by asking the
/// collision world what overlaps it, because these are solids — you can't be
/// inside one, only on top of it. "Landed" itself is read off the player's own
/// body (on the ground this frame, in the air the last one), so it's the same
/// touchdown the player felt, and somebody already standing there never fires
/// it.
pub fn player_landed_on(
    centre: Vec2,
    size: Vec2,
    entities: &Res<Entities>,
    ...
    let left = centre.x - size.x / 2.0;
    let right = centre.x + size.x / 2.0;
    entities
