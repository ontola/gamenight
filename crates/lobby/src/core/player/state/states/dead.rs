...
pub fn handle_player_state(
    ...
    for (player_ent, (state, animation, _killed_player, player_idx)) in entities.iter_with((
        ...
        if state.age >= 80 {
            // If only one player in match, we won't score / transition rounds,
            // so respawn player — GameNight's always-on lobby arena wants the
            // exact same behavior regardless of player count: free-for-all
            // forever, nobody stays dead.
            if player_indices.bitset().bit_count() == 1 || lobby_mode.0 {
