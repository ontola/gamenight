...
/// When set, this session is GameNight's always-on lobby arena rather than a
/// scored match: rounds never end (see [`round_end`]) and dead players
/// always respawn immediately (see `core::player::state::states::dead`),
/// the same code path already used for solo practice — just no longer
/// gated to exactly one player.
#[derive(HasSchema, Clone, Copy, Default)]
pub struct LobbyMode(pub bool);
...
pub fn round_end(
    ...
    lobby_mode: ResMutInit<LobbyMode>,
) {
    // GameNight's always-on lobby arena: never score, never end a round —
    // deaths respawn immediately (see the dead-state despawn gate), no
    // matter how many other players are still alive.
    if lobby_mode.0 {
        return;
    }
