...
impl AssetServer {
    ...
    pub async fn load_asset_bytes(&self, loc: AssetLoc, force: bool) -> anyhow::Result<Cid> {
        ...
        if !force {
            ...
            if let Some(cid) = cid {
                // GAMENIGHT PATCH: asset_ids may contain a cooked asset ID.
                // Only return it when the raw-byte store actually has that key.
                if self.store.asset_data.contains_key(&*cid) {
                    return Ok(*cid);
                }
            }
        }
